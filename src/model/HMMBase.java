...
public abstract class HMMBase {
	...
	public void loadModelsFromIndependentHMM(String folderName) {
		...
		for (String modelFilename : files) {
			if (modelFilename.equals(".") || modelFilename.equals("..")
					|| modelFilename.equals("vocab.txt"))
				continue;
			try {
				BufferedReader br = new BufferedReader(new FileReader(
						folder.getAbsolutePath() + "/" + modelFilename));
				try {
					nrStates = Integer.parseInt(br.readLine());
					nrStatesWithFake = nrStates;
					int vocabSizeFromModel = Integer.parseInt(br.readLine());
					if (vocabSizeFromModel != Corpus.corpusVocab.get(0).vocabSize) {
						System.err.println("Error: vocab size from model : "
								+ vocabSizeFromModel + " from dictionary : "
								+ Corpus.corpusVocab.get(0).vocabSize);
						System.exit(-1);
					}
					br.readLine();
					...
					if (nrStates != splitted.length) {
						...
						throw new RuntimeException(
								"Loading model, Initial parameters not matching number of states");
					}
					...
					for (int i = 0; i < nrStates; i++) {
						...
						if (nrStatesWithFake != splitted.length) {
							br.close();
							System.err
									.format("For transition: nrStates=%d, from file=%d\n",
											nrStatesWithFake, splitted.length);
							throw new RuntimeException(
									"Loading model, transition parameters not matching number of states");
						}
						for (int j = 0; j < splitted.length; j++) {
							...
							this.param.transition.get(modelCount).set(j, i,
									prob);
						}
					}
					br.readLine();
					...
					for (int i = 0; i < nrStates; i++) {
						...
						if (Corpus.corpusVocab.get(0).vocabSize != splitted.length) {
							...
							System.err.format("nrStates=%d, from file=%d\n",
									...
									splitted.length);
							throw new RuntimeException(
									"Loading model, obs parameters not matching number of states");
						}
						for (int j = 0; j < splitted.length; j++) {
							double prob = Double.parseDouble(splitted[j]);
							if (prob <= 1e-5) {
			...
			System.out
					.format("Layer %d, Model loaded successfully with %d states and %d observations \n",
							modelCount, nrStates,
							Corpus.corpusVocab.get(0).vocabSize);
			modelCount++;
		}
	...
	*/
	...
	public boolean isParamConverged() {
		//updateL1Diff();
		boolean result = true;
		if(param.l1DiffInitialMax > Config.precision) {
			result = false;
		}
		if(param.l1DiffTransitionMax > Config.precision) {
			result = false;
		}
		return result;
	}
